...
def load_data(data_path):
    return np.loadtxt(data_path, dtype=float, delimiter=",")
### TODO 3.1
...
def calculate_distances(data, centers):
    return np.sum((data[:, np.newaxis, :] - centers[np.newaxis, :, :])**2, axis=-1)
...
def update_centers(data, labels, K):
    ...
    np.add.at(x, labels, data)
    #operands could not be broadcast together with shapes (3,2) (3,)
    # this will start matching from right causing problems
    counts = np.pad(counts, ((0, K-len(counts))), mode="constant", constant_values=1)#if some label has not points it wont get included in counts by np.unique
    # we want counts to be (k, 1) so add new axis before
    return x/counts[:, np.newaxis]
